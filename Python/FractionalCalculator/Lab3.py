...
def SumOrSub(val1,val2,operat):
    try:
        if(operat =="+"):
            return proxy.summation(val1,val2)
        elif(operat == "-"):
            return proxy.difference(val1,val2)
        ...
        mb.showerror("Ошибка подключения","Нет подключения к серверу")
        return
...
def ArithmeticOperation(index1,index2,operat):
    ...
    try:
        if(operat == ":"):
            MassDecimal.append([DefinSign(val1_1[0], val2_1[0]) + proxy.multiplication(val1_1[1], val2_2), proxy.multiplication(val2_1[1], val1_2)])
        elif(operat =="*"):
            MassDecimal.append([DefinSign(val1_1[0], val2_1[0]) + proxy.multiplication(val1_1[1], val2_1[1]), proxy.multiplication(val2_2, val1_2)])
        elif(operat == "+" or operat == "-"):
            denominator = proxy.multiplication(val1_2, val2_2)
            val1 =proxy.multiplication(val1_1[1], val2_2)
            val2 =proxy.multiplication(val2_1[1], val1_2)
...
def Cal():
    ...
    for num in arr1:
        ...
        try:
            numerator = numerator[0] + proxy.summation(proxy.multiplication(wholePart, denominator), numerator[1])
        except xmlrpc.client.Fault as err:
            print("A fault occurred")
            print("Fault code: %d" % err.faultCode)
            print("Fault string: %s" % err.faultString)
